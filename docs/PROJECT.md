...
- Every character's numbers can be changed in the game: Mods > Multiverse Powers > Config, first *Client* or
  *Server* (the host or an operator only), then a tab per character, a part per ability that folds open and shut,
...
- Homing, auto-aim and area hits (the Giant Hands, the Air Strike, the Light Cage's slam, the landing slam,
