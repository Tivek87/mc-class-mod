...
- **Pak licht op, alleen terwijl de ring werkt:** de lantaarn op je borst gloeit als een kern, groene energielijnen lopen door het pak, vooral via je rechterarm naar de ring, en de ring straalt fel. Hoe harder de ring werkt, hoe feller. Een ring in rust laat het pak gewoon.
- **Paneel:** toont de power; loopt de ring vanzelf leeg (vliegen, schild, koepel, laser), dan ook hoe snel (`73 -2.5/s`), en in de lucht hoeveel seconden vliegen er nog over zijn (`73 -6.7/s 11s`).
...
- **Click / Tap (Light Shield, aan/uit):**
  - **Actie:** Een tik zet een klein rond schild van hard licht voor je in je kijkrichting, vastgehouden door je linkerhand. Het blijft staan tot je nog een keer tikt.
  ...
  - **Kosten:** 0,5 power per seconde zolang het staat.
  - **Stopt:** Nog een tik, recharge of een lege ring.
- **Hold na 2 sec (Forcefield Dome / Krachtveld-Koepel):**
  - **Actie:** Het schild vouwt open tot een koepel van hard licht rondom je hele lichaam (360°), zolang je rechtsklik inhoudt.
  ...
  - **Kosten:** 1,5 power per seconde. Een schild dat al aan stond wacht onder de koepel en kost dan niets.
...
- **Kosten:** 10 power (tik) tot 20 power (volle lading). De ring betaalt pas als hij wordt afgevuurd.
...
- **Niet in de lucht:** de lantaarn staat op de grond, dus eerst landen, dan opladen.
...
- **Vliegen:** Vooruit inhouden = snelheid opbouwen in je kijkrichting, tot **50 blokken per sec** (50% sneller dan een elytra met raketten). Loslaten = uitglijden tot zweven. Springen = stijgen, bukken = zakken, links/rechts = opzij, achteruit = terugdrijven. Je neemt je snelheid mee in bochten.
...
- **Kosten:** Opstijgen minstens **5 power**. Vliegen kost **6,7 power per sec** (100 / 15): een volle ring houdt je **max 15 sec** in de lucht. Alles wat je tijdens het vliegen doet kost daar bovenop.
...
- **Ring leeg in de lucht:** Je zakt langzaam omlaag met je armen omhoog, zonder sturen, en landt zonder schade.
- **Vechten in de lucht (dezelfde knoppen):**
  ...
  - *Tik rechts:* het schild wordt een puntige, gestroomlijnde **ramkegel** voor je, linkervuist vooruit. Houdt 70% van voren tegen; wat je raakt wordt weggeslingerd en krijgt **2 harten + 3 harten per blok per tick snelheid** (ongeveer 9,5 harten op topsnelheid). Zelfde wezen pas na 0,6 sec opnieuw.
