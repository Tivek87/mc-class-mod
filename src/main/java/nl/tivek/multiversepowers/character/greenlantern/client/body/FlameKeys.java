...
abstract class FlameKeys extends FlameCurves {
    ...
    static final Pose REST = rest(pose(RechargeAnimation.HAND_RIGHT.x(), RechargeAnimation.HAND_RIGHT.y(),
            ...
            0.0F, 0.0F));
    // The guard in a fighting stance: feet apart, knees soft.
    static final Pose READY = stance(GUARD, 0.08F, 0.3F);
    private static final Pose BRACED = stance(pose(0.39, -0.45, -0.82, -0.17, 0.07, -0.99, 0.04, 1.0, 0.06, -0.3,
            -0.6, -0.8, 1.0F, 0.0F, 25.0F, 0.2F, 0.35F), 0.25F, 0.55F);
    private static final Pose SWIRL = stance(pose(0.36, -0.58, -0.72, -0.05, -0.8, -0.55, 0.0, 0.55, -0.8, -0.3, -0.6,
            -0.8, 1.0F, 0.0F, 20.0F, -0.05F, 0.1F), 0.3F, 0.5F);

    static final Map<FlameMove, Keyframes.Key[]> MOVES = new EnumMap<>(FlameMove.class);

    static {
        equip();
        FlameAttacks.fill(MOVES);
    ...
    private static void equip() {
        ...
        MOVES.put(FlameMove.EQUIP, new Keyframes.Key[] {
                ...
                key(FlameMove.EQUIP.ticks(), false, READY) });
    }
    ...
    // Never quite still: it breathes, the weight drifts from foot to foot and the nozzle wanders a little.
    static Pose idle(float time, float amount) {
        ...
        n[1] += amount * 0.012F * breath;
        n[2] += amount * 0.006F * drift;
        n[3] += amount * 0.02F * sway;
        n[4] += amount * 0.014F * breath + amount * 0.01F * drift;
        n[14] += amount * 0.05F * sway;
        n[15] += amount * 0.02F * breath;
        n[Pose.ROLL] += amount * 0.015F * weigh;
        n[Pose.SQUAT] += amount * 0.03F * (0.5F + 0.5F * breath);
        n[16] += amount * 0.08F * weigh;
        return Pose.of(n);
    }
