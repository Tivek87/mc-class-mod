...
final class SwordPoses {
    ...
    record Pose(Vec3 hand, Vec3 blade, Vec3 edge, Vec3 shield, Vec3 face, Vec3 top, float twist, float lean,
        ...
        // Where the numbers of the blade's edge are in numbers(), and those of the shield: its middle, face and top.
    ...
    static Pose at(SwordMove move, float t, float time, Pose from) {
        return switch (move) {
            case FLURRY -> from.mix(flurry(t), (float) Ease.smooth(t / BLEND_IN));
            case CHARGE -> from.mix(charge(time), (float) Ease.smooth(t / BLEND_IN));
    ...
    private static Pose keyed(Key[] keys, float t, Pose from, Pose rest, float settle) {
        ...
        for (int k = 1; k < count; k++) {
            values[k] = aligned(values[k], values[k - 1]);
        }
    ...
    static float shieldGrown(SwordMove move, float t) {
        return move == SwordMove.EQUIP ? (float) Ease.smooth((t - 2.0F) / 7.0F) : 1.0F;
    }
