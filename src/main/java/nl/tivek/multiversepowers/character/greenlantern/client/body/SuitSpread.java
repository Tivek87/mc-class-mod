...
public final class SuitSpread implements VertexConsumer {
    public interface Field {
        // must return 0 on the edge, negative outside, positive inside: callers rely on the sign
    ...
    public SuitSpread(VertexConsumer inner) {
        this.inner = inner;
    }

    public void render(ModelPart part, Field field, PoseStack pose, int light, int overlay) {
        if (!part.visible) {
            return;
        }
        pose.pushPose();
    ...
    public void seam(MultiBufferSource buffers, float strength) {
