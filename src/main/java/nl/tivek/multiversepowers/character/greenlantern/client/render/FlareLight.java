...
public final class FlareLight {
    ...
    public static final Vector3f UP_HIGH = new Vector3f(0.42F, 0.24F, -0.78F);
    ...
    public record Going(int owner, double clock, Vec3 center, double radius) {
    }

    /** Where the ring is while its owner holds it up for a flare: high over his head, a little to his right. */
    public static Vec3 ring(Entity owner, float partialTick) {
     ...
     */
    public static Vec3 ownRing(Camera camera) {
     ...
     */
    public static void draw(LanternPainter painter, Vec3 at, Vec3 facing, double clock, boolean own) {
        ...
        Vec3 forward = Vectors.spin(flat, Vectors.UP, 0.04 * clock * clock / gather);
        double size = LANTERN_SCALE * scale * Ease.backOut(clock / LANTERN_GROWS);
        ...
        if (size > 0.01 && apart < 1.0) {
            ConstructPainter.Frame frame = ConstructPainter.Frame.of(foot, forward, Vectors.UP, size);
            if (since < 0.0) {
                ...
                double fresh = 0.6 * (1.0 - Ease.smooth(clock / LANTERN_GROWS));
    ...
    private static void gathering(LanternPainter painter, Vec3 heart, double t, double scale) {
        painter.flare(heart, (0.15 + 0.7 * t * t) * scale, 0.6 + 0.4 * t);
        double clock = t * LightFlare.GATHER_TICKS;
        for (int k = 0; k < 18; k++) {
            double cycle = Mth.frac(clock / (5.0 - 2.0 * t) + Noise.of(k, 81, 0));
            Vec3 way = Noise.direction(k, 81 + (int) (clock / 4.0));
        ...
        for (int k = 0; k < 3; k++) {
            ...
            Vec3 tip = Noise.direction(k, 85);
            Vec3 a = tip.cross(Vectors.UP).lengthSqr() < 1.0E-6 ? new Vec3(1.0, 0.0, 0.0)
                    : tip.cross(Vectors.UP).normalize();
            Vec3 b = tip.cross(a).normalize();
            double radius = (2.2 * (1.0 - close) + 0.2) * scale;
            painter.circle(heart, a, b, radius, 0.03 * scale, 0.18 * scale,
                    Colors.alpha(0.8 * close * (0.4 + 0.6 * t)), Colors.alpha(0.35 * close));
        }
    }
     ...
     */
    private static void burst(LanternPainter painter, Vec3 at, double since, double scale) {
        ...
        if (fade > 0.0) {
            ...
            // Rays shooting out.
            for (int k = 0; k < 20; k++) {
                Vec3 way = Noise.direction(k, 83);
                double reach = (1.5 + 6.0 * Noise.of(k, 83, 5))
            ...
            painter.circle(at, east, south, radius, 0.08, 0.6, Colors.alpha(fade),
                    Colors.alpha(0.5 * fade));
            painter.circle(at, east, up, radius, 0.06, 0.4, Colors.alpha(0.7 * fade),
                    Colors.alpha(0.35 * fade));
            painter.circle(at, south, up, radius, 0.06, 0.4, Colors.alpha(0.7 * fade),
                    Colors.alpha(0.35 * fade));
        }
        ...
        if (ground > 0.0 && scale >= 1.0) {
            ...
            painter.circle(feet, east, south, 0.5 + 12.0 * run, 0.12, 0.8, Colors.alpha(0.8 * ground),
                    Colors.alpha(0.4 * ground));
        }
        // Specks of its light drifting down and fading.
        double left = 1.0 - since / LightFlare.BURST_TICKS;
        for (int k = 0; k < MOTES; k++) {
            Vec3 way = Noise.direction(k, 87);
            double out = (1.0 - Math.exp(-since * 0.25)) * (2.0 + 3.0 * Noise.of(k, 87, 3)) * scale;
    ...
    static float raised(Entity player, float partialTick) {
        ...
        return (float) (Ease.smooth(flare.clock() / 2.5) * (1.0 - Ease.smooth((flare.clock()
                - down + 5.0) / 5.0)));
    }
