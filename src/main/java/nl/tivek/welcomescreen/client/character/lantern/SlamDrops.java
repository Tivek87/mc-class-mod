...
final class SlamDrops {
    ...
    static Vec3 boot(ConstructPainter painter, Moment m) {
    ...
    static Vec3 lantern(ConstructPainter painter, Moment m) {
     ...
     */
    static Vec3 anchor(ConstructPainter painter, Moment m) {
    ...
    static Vec3 mace(ConstructPainter painter, Moment m) {
        ...
        Frame frame = new Frame(center, ConstructPainter.spin(m.right(), axis, turn),
    ...
    static Vec3 barbell(ConstructPainter painter, Moment m) {
     ...
     */
    static Vec3 bell(ConstructPainter painter, Moment m) {
        ...
        if (m.struck() && m.apart() <= 0.0) {
            ...
            frame = new Frame(frame.center(), ConstructPainter.spin(frame.right(), m.forward(), ring),
            ...
            for (int k = 0; k < 3; k++) {
                ...
                if (age > 0.0 && age < 8.0) {
                    ...
                    painter.circle(frame.at(0.0, 0.9, 0.0), m.right(), m.forward(),
                            (1.3 + 0.35 * age) * frame.scale(), 0.05, 0.25, ConstructPainter.alpha(0.8 * fade),
    ...
    private static void crack(ConstructPainter painter, Frame frame, int ring, int from, double strength) {
        Vec3 last = null;
        for (int k = 0; k < 6; k++) {
            int row = ring + (k & 1);
     ...
     * a moment.
     */
    static Vec3 sword(ConstructPainter painter, Moment m) {
