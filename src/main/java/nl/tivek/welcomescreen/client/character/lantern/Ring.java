...
final class Ring {
    ...
    private static final ResourceLocation METAL = ResourceLocation.fromNamespaceAndPath(WelcomeScreenMod.MODID,
            "textures/entity/ring_metal.png");
    /** Unlit solid colour: the lantern is drawn with it. */
    static final RenderType BAND = RenderType.create("welcomescreen_ring", DefaultVertexFormat.POSITION_COLOR,
     ...
     */
    static final RenderType HALO = RenderType.create("welcomescreen_ring_halo", DefaultVertexFormat.POSITION_COLOR,
     ...
     * light leaves it.
     */
    static Vector3f stone(boolean slim) {
     ...
     */
    static void draw(PoseStack poseStack, MultiBufferSource buffers, int light, boolean slim, float strength,
        ...
        int stone = mix(mix(STONE_EMPTY, STONE_FULL, power), GLINT_FULL, 0.35F * burn);
        ...
        solid(metal, pose, g - 0.97F, 8.56F, -0.66F, g - 0.9F, 8.8F, -0.42F,
                mix(GLINT_EMPTY, GLINT_FULL, power), FULL_BRIGHT);
        if (burn > 0.01F) {
    ...
    /** The colour between {@code a} (at 0) and {@code b} (at 1). */
