...
public final class ClientLooks {
     ...
     */
    record Uniform(float arm, float torso, float rest, float mask, float ring, boolean complete) {
    ...
    static Uniform uniform(Entity player, float partialTick) {
        ...
        if (look.takingOff()) {
            ...
            return new Uniform(1.0F - smooth((u - 0.6F) / 0.35F), 1.0F - smooth((u - 0.35F) / 0.3F),
                    1.0F - smooth((u - 0.1F) / 0.35F), 1.0F - smooth(u / 0.15F), 1.0F, false);
        }
        ...
        return new Uniform(smooth(s / 0.28F), smooth((s - 0.25F) / 0.35F), smooth((s - 0.42F) / 0.4F),
                smooth((s - 0.86F) / 0.14F), 1.0F, false);
    }
     ...
     */
    static float departure(Entity player, float partialTick) {
        Look look = LOOKS.get(player.getId());
