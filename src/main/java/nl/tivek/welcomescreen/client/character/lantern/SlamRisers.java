...
/**
 * The landing-slam constructs that burst out of the ground or stand up out of it (see {@link SlamPainter}): two rings
 * of crystal spikes, a pillar that topples over, and the lantern emblem that falls flat on its face. Unless it says
 * otherwise a shape stands on y = 0, in blocks at scale 1.
 */
final class SlamRisers {
    ...
    /** A spike: a six-sided crystal narrowing up to a point, with two smaller crystals growing out of its foot. */
     ...
     * out shoot up out of it one after the other, leaning outwards, overshooting a little and swaying as they settle,
     * with chunks of ground flying and their points glinting; at the end they sink back into the ground.
     */
        ...
        for (int ring = 0; ring < counts.length; ring++) {
            ...
            for (int i = 0; i < counts[ring]; i++) {
                ...
                if (rise >= 1.0 && back <= 0.0) {
                    // A glint running over the points, one after the other.
    ...
    // Where the drums of its shaft start and end, from the base up.
    private static final double[] DRUMS = { 0.40, 1.60, 2.80, 4.00 };

    /** The foot of the pillar: a square plinth and a round moulding on it. */
    ...
    /** The three drums its shaft is built of, each round with ribs down it and a bright joint at its foot. */
    private static final Shape[] PILLAR_DRUMS = { drum(0), drum(1), drum(2) };
    /** The top of the pillar: a flaring capital and a square slab on it. */
     ...
     * crashes down flat on the ground, its middle where it strikes. It bounces once, and the drums of its shaft come
     * apart and roll a little.
     */
    static Vec3 pillar(ConstructPainter painter, Moment m) {
        ...
        Frame frame = new Frame(base.subtract(along.scale((1.0 - up) * height)).add(0.0, bounce, 0.0), m.right(),
                along, ahead, s);
        if (up > 0.02) {
            double apart = m.struck() ? ConstructPainter.smooth(since / 4.0) : 0.0;
            SlamPainter.piece(painter, PILLAR_BASE, frame, m);
            for (int k = 0; k < PILLAR_DRUMS.length; k++) {
                // Each drum slides a little further along than the one before it, and rolls about the shaft.
    ...
    // Half the height of the emblem at scale 1.
    private static final double EMBLEM_HALF = 1.4;

    /**
     * The lantern emblem, standing up in the plane of x and y round its middle: a ring with bevelled edges, and a bar
     * with bevelled ends above and below it, each with a bright raised face.
     */
    ...
    /** A bar of the emblem round height {@code y}, its ends bevelled. */
    ...
    /**
     * The lantern emblem: it stands up on its bottom edge, then falls flat on its face away from him, so it lands with
     * its middle on the middle of the wave. It bounces once off the ground and its light pulses out over it.
     */
    static Vec3 emblem(ConstructPainter painter, Moment m) {
            ...
            for (int k = 0; k < 2; k++) {
                ...
                if (age > 0.0 && age < 7.0) {
                    ...
                    painter.circle(middle.add(0.0, 0.05, 0.0), m.right(), m.forward(), (0.9 + 0.5 * age) * s, 0.06,
                            0.3, ConstructPainter.alpha(0.9 * fade), ConstructPainter.alpha(0.45 * fade));
                }
            }
        }
        return middle;
