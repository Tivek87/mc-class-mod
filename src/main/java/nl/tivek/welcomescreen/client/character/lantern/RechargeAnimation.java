...
public final class RechargeAnimation {
    ...
    static float press(float t) {
        ...
        if (t < WIND) {
            return 0.3F * smooth((t - RAISED) / (WIND - RAISED));
        }
        if (t < HIT) {
        ...
        return 1.0F - smooth((t - BACK) / (GONE - 2.0F - BACK));
    }
    ...
    static float wind(float t) {
        ...
        if (t < WIND) {
            return smooth((t - RAISED) / (WIND - RAISED));
        }
        float in = (t - WIND) / (HIT - WIND);
    ...
    static float shown(float t) {
        return smooth(t / POP) * (1.0F - smooth((t - (GONE - POP)) / POP));
    }
    ...
    static float glow(float t) {
        if (t < HIT) {
            return 0.25F + 0.45F * smooth((t - RAISED) / (HIT - RAISED));
        }
        if (t < BACK) {
            return 1.0F + 1.9F * kick(t);
        }
        return 1.0F - smooth((t - BACK) / (GONE - BACK));
    }
     ...
     */
    static void arm(PoseStack pose, MultiBufferSource buffers, int light, AbstractClientPlayer player,
