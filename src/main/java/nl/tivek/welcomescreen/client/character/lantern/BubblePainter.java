...
final class BubblePainter {
    ...
    static final int POUND_TICKS = 16;
    ...
    static void draw(ConstructPainter painter, ConstructPayload bubble, Vec3 center, double solid, double broken,
        ...
        if (bubble.variant() == LightBubble.SMASHING && speed > 0.15) {
            ...
            Vec3[] across = ConstructPainter.Basis.of(way);
    ...
    static boolean last(ConstructPayload pound) {
     ...
     */
    static void pound(ConstructPainter painter, ConstructPayload pound, double since) {
        ...
        for (int k = 0; k < (last ? 3 : 2); k++) {
            ...
            painter.circle(at, east, south, 0.5 + reach * wave, 0.08, 0.6, ConstructPainter.alpha(fade),
        ...
        for (int i = 0; i < cracks; i++) {
            double angle = Math.PI * 2.0 * (i + 0.4 * ConstructPainter.noise(pound.id(), i, 1)) / cracks;
            double length = reach * (0.5 + 0.4 * ConstructPainter.noise(pound.id(), i, 2)) * grow;
            Vec3 from = at;
            for (int s = 1; s <= 3; s++) {
                double bend = angle + (ConstructPainter.noise(pound.id(), i, 3 + s) - 0.5) * 0.5;
        ...
        if (last && since < 8.0) {
            ...
            painter.haze(at.add(0.0, high * 0.5, 0.0), new Vec3(wide, 0.0, 0.0), new Vec3(0.0, high, 0.0),
                    new Vec3(0.0, 0.0, wide), ConstructPainter.BRIGHT, 0.5 * burst);
        }
    }
}
